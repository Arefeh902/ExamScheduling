...
class Penalty:
    TWO_EXAMS_IN_ONE_DAY = 1_000
    TWO_CONSECUTIVE_EXAM = 20
    THREE_CONSECUTIVE_EXAM = 70
    EXAM_ON_HOLIDAY = 0
    SINGLE_DAY_REST = 0
...
def calculate_penalty_of_student(schedule: Schedule, student: Student) -> int:
    ...
    two_in_one_day = sum([max(d-1, 0)*d//2 for d in days])
    ...
    single_day_rest = sum([days[i]*days[i+2] for i in range(len(days)-2) if days[i+1] == 0])
